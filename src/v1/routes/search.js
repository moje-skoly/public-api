...
import geocode from "../model/geocoding";
...
const search = (req, res) => {
  geocode(req.params.address, (lon, lat, addresses) => {
    if (lon !== null) {
      ...
      find(lon, lat, radius, type, params)
        .then(schools => {
          res.json({
            ...
            schools: schools.hits.hits.map(hit =>
              Object.assign(
                {
                  _id: hit._id,
                  distance: hit.sort[0]
...
export default (app, middleware) => {
  ...
  router.get(
    "/:address/:radius/:type",
    middleware.validateRequestBody(Schema),
    search
  );
  router.post(
    "/:address/:type",
    middleware.validateRequestBody(Schema),
    search
  ); // POST may contain the body of the request
  router.post(
    "/:address/:radius/:type",
    middleware.validateRequestBody(Schema),
    search
