...
const getSchools = (req, res) => {
  find(...req.body.ids)
    .then(schools => {
      const foundSchools = schools.docs
        .filter(doc => doc.found)
        .map(doc => Object.assign({ _id: doc._id }, doc._source));
      const notFoundSchools = schools.docs
        .filter(doc => !doc.found)
        .map(doc => doc._id);
      res.status(StatusCode.OK).json({
        success: true,
...
export default (app, middleware) => {
  ...
  router.post("/", getSchools); // POST may contain the body of the request => so multiple schools can be requested at once
